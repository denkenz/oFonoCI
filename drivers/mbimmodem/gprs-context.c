					...
					ofono_gprs_context_cb_t cb, void *data)
{
	...
	if (mbim_device_mbimex_version_at_least(mbimex_version, 3, 0)) {
		mbim_message_set_arguments(message, "uuuuu16yuTTTT",
						cid, 0, 0, 0, 0, mbim_context_type_internet,
						0, MBIM_TLV_TYPE_WCHAR_STR, NULL,
						MBIM_TLV_TYPE_WCHAR_STR, NULL,
						MBIM_TLV_TYPE_WCHAR_STR, NULL,
						MBIM_TLV_TYPE_INVALID, NULL);
	} else {
				...
				ofono_gprs_context_cb_t cb, void *data)
{
	...
	if (mbim_device_mbimex_version_at_least(mbimex_version, 3, 0)) {
		mbim_message_set_arguments(message, "uuuuu16yuTTTT",
			...
			mbim_context_type_internet,
			0, /* MBIMMediaTypeNone */
			MBIM_TLV_TYPE_WCHAR_STR, ctx->apn,
			MBIM_TLV_TYPE_WCHAR_STR, username,
			MBIM_TLV_TYPE_WCHAR_STR, password,
			MBIM_TLV_TYPE_INVALID, NULL);
	} else {
		mbim_message_set_arguments(message, "uusssuuu16y",
			ctx->cid,

						...
						void *user)
{
	...
	while (mbim_message_iter_next_entry(&iter, &provider_id, &provider_state,
		...
		strncpy(list[i].name, provider_name, OFONO_MAX_OPERATOR_NAME_LENGTH);
		list[i].name[OFONO_MAX_OPERATOR_NAME_LENGTH] = '\0';
...
static void mbim_register_state_set_cb(struct mbim_message *message, void *user)
{
	...
	if (mbim_message_get_error(message) != 0)
		CALLBACK_WITH_FAILURE(cb, cbd->user);
	else
		CALLBACK_WITH_SUCCESS(cb, cbd->user);
}

static void mbim_register(struct cb_data *cbd, enum mbim_register_type reg_type,
				const char *provider_id)
{
	static const uint32_t data_class = MBIM_DATA_CLASS_GPRS |
						...
						MBIM_DATA_CLASS_LTE;
	struct netreg_data *nd = cbd->data;
	ofono_netreg_register_cb_t cb = cbd->cb;
	struct mbim_message *message;
...
{
	struct netreg_data *nd = ofono_netreg_get_data(netreg);
	struct cb_data *cbd = cb_data_new(cb, nd);

	cbd->user = data;

	mbim_register(cbd, MBIM_REGISTER_TYPE_AUTOMATIC, NULL);
}
				...
				ofono_netreg_register_cb_t cb, void *data)
{
	...
	mbim_register(cbd, MBIM_REGISTER_TYPE_MANUAL, provider_id);
}
